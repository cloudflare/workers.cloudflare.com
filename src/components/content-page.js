...
const ContentPage = ({ image, link, title, description, cta, children }) => {
  ...
  return (
    <div
      className={tw`antialiased text-black dark:text-white mx-4`}
      hidden
      ref={content}
    >
      <div className={tw`md:max-w-4xl mx-auto pt-16`}>
        <div className={tw`mt-16 mb-8 flex items-start align-middle`}>
          ...
          {link ? (
            <a
              className={[`Button Button-is-primary`, tw`text-xl mt-2`].join(
                " "
              )}
              href={link.url}
            >
      ...
      <div className={tw`md:max-w-4xl mx-auto pt-8`}>{children}</div>
    </div>
  )
}
